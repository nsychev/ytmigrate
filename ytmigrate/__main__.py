...
def reupload(url: str, path: str, group_id: int | None, album_id: int | None, publish: bool):
...
# @click.option("--publish", is_flag=True, help="Make the video public")  # TODO
...
def main(group_id: int | None, album_id: int | None, debug: bool, url: str, publish: bool = False):
    ...
    if debug:
        ...
        reupload(url, str(target), group_id, album_id, publish)
    else:
        with tempfile.TemporaryDirectory() as temp_dir:
            reupload(url, temp_dir, group_id, album_id, publish)
